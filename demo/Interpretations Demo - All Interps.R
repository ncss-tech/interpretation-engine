...
### 
### revised by AGB 2021-03-25 to use new R package InterpretationEngine
...
# set to the InterpretationEngine package directory (e.g. as RStudio project)
...
sf.studyarea <-
  sf::st_read(dsn = "demo//Utah_County_Boundaries-shp", layer = "Counties") %>%
...
paths.vf <- list.files(path = paste0("inst/extdata/Input/VF/NASIS Property Rasters/", projectcode), full.names = T)
paths.hsg <- list.files(path = paste0("inst/extdata/Input/HSG/", projectcode), pattern = "tif$", full.names = T)
paths.svi <- list.files(path = paste0("inst/extdata/Input/SVI/", projectcode), pattern = "tif$", full.names = T)
...
if(!dir.exists(file.path(output_dir, projectcode))) 
  dir.create(file.path(output_dir, projectcode))

writeRaster(out.hsg,
            overwrite = TRUE,
            file.path(output_dir, projectcode, paste0(projectcode, "_hsg.tif")))

writeRaster(out.svi,
            overwrite = TRUE,
            file.path(output_dir, projectcode, paste0(projectcode, "_svi.tif")))
...
writeRaster(out.dwb$maxfuzz,
            ...
            file.path(
              output_dir,
              projectcode,
              paste0(projectcode, "_dwbRating.tif")
            ))

writeRaster(out.vf$fuzzvf,
            overwrite = TRUE,
            file.path(output_dir, projectcode, paste0(projectcode, "_vfRating.tif")))

### factoring is currently outside of the functions, because saving these doesn't work:
### 
# TODO: Fix raster attribute table
#
...
writeRaster(r, overwrite = TRUE, file.path(output_dir, projectcode, paste0(projectcode, "_dwbClass.tif")))
writeRaster(r2, overwrite = TRUE, file.path(output_dir, projectcode, paste0(projectcode, "_vfClass.tif")))
