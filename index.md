...
further interpolated using
[`approxfun()`](https://rdrr.io/r/stats/approxfun.html) and
[`splinefun()`](https://rdrr.io/r/stats/splinefun.html). NASIS hedges
...
4.  Load hedge and operator functions into each decision node of
    `data.tree` object
5.  Use wrapper function
    ([`interpret()`](http://ncss-tech.github.io/interpretation-engine/reference/interpret.md))
    to:
    - Send properties to evaluation functions
    - Combine fuzzy values via operators and hedges to generate a final
      fuzzy rating
...
The
[`interpret()`](http://ncss-tech.github.io/interpretation-engine/reference/interpret.md)
function takes a data.frame or SpatRaster where column or variable names
correspond to input property names. Property names can be extracted
using
[`getPropertySet()`](http://ncss-tech.github.io/interpretation-engine/reference/getPropertySet.md)
and then made into compatible names using the base R function
...
- some properties return an RV, some {low,RV,high}:
  - alternately, allow for selection of one specific value that can be
    customized on property basis.
