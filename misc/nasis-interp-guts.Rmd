...
```{r, fig.width=6, fig.height=4}
# arbitrary curve (showing control points)
e <- evals[evals$evalname == '*Storie Factor C Slope 0 to 100%', ]
plotEvaluation(e, xlim = c(0, 200), pch = 3)
plotEvaluation(e, xlim = c(0, 8), pch = 3)
...
# arbitrary linear (showing control points)
e <- evals[evals$evalname == 'GRL - EC maximum in depth 25 to 50 cm (NV)',]
plotEvaluation(e, xlim = c(0, 1000), pch = 3)

# linear
e <- evals[evals$evalname == 'Slope 0 to >15%', ]
plotEvaluation(e, c(-1,20))
...
## beta
e <- evals[evals$evalname == 'Surface pH - GCPI (Referenced)',]
plotEvaluation(e)

## gauss (inverted)
e <- evals[evals$evalname == 'Soil Reaction (pH between 5.6 & 7.6) Surface Layer',]
plotEvaluation(e)

## triangle
e <- evals[evals$evalname == 'ISU Slope <15%',]
plotEvaluation(e)

## PI (inverted)
e <- evals[evals$evalname == 'BW and BC soil reaction',]
plotEvaluation(e)

## crisp expression
e <- evals[evals$evalname == 'GRL-Frost Action = moderate', ]
extractCrispExpression(e$eval)(c('moderate','none'))
```
...
# for "crisp expressions" there is no curve to plot, but the evaluation can still be extracted and run
e <- evals[evals$evalname == 'WICCPI - Interpretable Component', ]
WICCPIInterpretableComponent <- extractCrispExpression(e$eval)
# the function template takes x as an argument, in this case x corresponds to output from PROPERTY script INTERPRETABLE COMPONENT
WICCPIInterpretableComponent(x = c("series", "miscellaneous area", "taxadjunct", "miscellaneous area"))
```
