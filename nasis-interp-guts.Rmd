...
args <- list(prop_name='AWC, 0-50CM OR FIRST RESTRICTIVE LAYER', cokey='1842387')
...
```{r}
(props <- lookupProperties(unique(ps$propname), coiid='1842387'))

# join with properties
z <- join(ps, props, by='propname', type='left')
```
...
Get all properties that this interpretation requires, for a given component.
Lookup is done by property name. This doesn't work for properties with special characters in the name (>).
