...
Get a table of evaluations, properties, and property IDs
```{r, fig.width=6, fig.height=4}
getPropertySet(dt)
```
...
ps <- getPropertySet(dt)
write.csv(unique(ps), file='nccpi-properties.csv', row.names = FALSE)
...
write.csv(unique(d), file='nccpi-table-representation.csv', row.names = FALSE)
```
...
options(width=500, max.print = 1e6)
sink(file = 'CCPI-WI-full-tree.txt')
print(dt, 'Type', 'Value', 'RefId', 'rule_refid', 'eval_refid', 'evalType', 'propname', 'propiid', limit=NULL)
sink()
...
ps <- getPropertySet(dt)
write.csv(unique(ps), file='CCPI-WI-properties.csv', row.names = FALSE)

d <- ToDataFrameTable(dt, 'name', 'Type', 'Value', 'RefId', 'rule_refid', 'eval_refid', 'evalType', 'propname', 'propiid')
write.csv(unique(d), file='CCPI-WI-table-representation.csv', row.names = FALSE)
